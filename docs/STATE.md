...
| Current notebook | **`09_fashion_mnist_capstone`** (NB 9 of 10) — **DONE** (committed + ff-merged to `chapter/12`). The visualization-first Fashion-MNIST capstone, in PyTorch. **9/10 shipped.** |
| Phase | **`notebook-commit` done** — NB 9 committed (`feat(12_neuralnetworks): notebook 09 — Fashion-MNIST capstone`) & ff-merged into `chapter/12_NeuralNetworks` (Rémy validated visually 2026-07-02; both reviewers no-BLOCK, all folds applied incl. the causal-conflation fix). **9/10 notebooks shipped.** **Between notebooks — awaiting Rémy's "go" to open NB 10 (the whole-course synthesis).** |
| Active branch | **`chapter/12_NeuralNetworks`** (NB 9 merged via ff-only; Fashion-MNIST loader @ `2acd9ab`; `deep` extra @ `08fbcf2`). `notebook/12_NeuralNetworks__09_fashion_mnist_capstone` is merged (deletable). |
| Active plan | NB 9 DONE (`docs/plans/12_NeuralNetworks__09_fashion_mnist_capstone.md`). Chapter: `docs/plans/chapter_12_NeuralNetworks.md` (APPROVED). Next: plan NB 10 (the whole-course synthesis) on Rémy's go. |
| Next concrete action | **Open & plan NB 10 — where ML goes next, and the whole course (the finale) — on Rémy's go.** `git switch chapter/12_NeuralNetworks && git switch -c notebook/12_NeuralNetworks__10_where_ml_goes_next`; phase `notebook-plan`. **Chapter plan §NB 10 (Rémy ✅):** the **ONE intentional exception** to the by-hand + executable-"Your turn" pattern — a **reflective close**, no new mechanism, no executable exercise. Two halves: (1) **where ML goes next** — CNN / RNN / transformer **named, not built** (c09/c10/c11), *motivated by NB 9's verdict* (a dense net flattens the image → CNNs add locality + weight-sharing; RNNs/transformers extend "learn representations" to sequences and to the attention behind today's LLMs); (2) **the whole-course synthesis** — the spine instance-based (KNN) → probabilistic (NB) → linear (LogReg) → partitions (DTree) → margins (SVM) → ensembles (RF → AdaBoost → GBM → XGBoost → LightGBM) → learned representations (MLP → NeuralNetworks), and the earned through-line **"there is no universal best model."** Close with a reflective **"Where to go next"** (directional prompts, **not** code — e.g. "which of the 12 methods would you reach for on a dataset from your own world, and why"; "which problems in your field have the spatial/sequential structure that calls for a CNN/transformer") + a whole-course **"What you built"** celebration. Mostly prose + **1–2 schematic figures** (each with a "Read the figure"). Stated as deliberate so it reads as an earned finale, not a thinned NB. **No `src/` change expected** (pytest stays 26). Draft cell-by-cell → ExitPlanMode. **Then chapter close: PR `chapter/12_NeuralNetworks → main` (`--no-ff`, PR #12) — the course finale — on Rémy's explicit go.** Build scripts in the **ephemeral** scratchpad ([[scratchpad-build-scripts-ephemeral]]). |
