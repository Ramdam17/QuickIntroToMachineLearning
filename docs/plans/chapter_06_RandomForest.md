...
| 2 | `notebook/06_RandomForest__02_decorrelating_trees` | **done** — built (22 cells, 2 figures: ρ vs `max_features` rising→saturating; ensemble-CV vs mean-individual-tree across `max_features`), both reviewers folded (pedagogy **PASS**; ml-expert **REVISE** → 1 MAJOR fixed: the gem's CV gain 0.947→0.957 is seed-fragile (flips on 2/6 seeds) → re-anchored on the **robust** signals (ρ 0.82→0.80 on every seed, individual trees equal 0.91), by elimination → decorrelation lowers the variance floor; CV framed within the ±0.01 seed band. MINORs: ρ "saturates" not "monotone", ρ = proxy for error-correlation noted, cell-12↔16 fence, moons reframed, exercise-2 enriched). The Var = ρσ² + (1−ρ)σ²/B law **derived from scratch** + Monte-Carlo-verified. No `src/` change (pytest 19). Rémy validated visually, merged to `chapter/06_RandomForest` |
| 3 | `notebook/06_RandomForest__03_out_of_bag` | **done** — built (20 cells, 2 figures: in-bag/OOB schematic; OOB-error vs test-error vs `n_estimators`), both reviewers folded (pedagogy **PASS**; ml-expert **REVISE** → 1 MAJOR fixed: the hand-vs-sklearn OOB gap was wrongly attributed to hard-vs-soft vote (soft == hard here, saturated leaf probs) → corrected to RNG (different bootstrap draws, ±1 pt on 398 pts) + MINORs: optimism quantified ~1–2 pts persistent (parallel, not converging), n=10 cited 0.349, `np.add.at` glossed). Anchors: (1−1/n)ⁿ 0.367 / empirical 0.368; hand OOB 0.962 ≈ sklearn `oob_score_` 0.955; sealed test 0.942; ~73 graders/point; OOB unreliable < ~25 trees (sklearn warns, P(never OOB)=0.63³≈0.25). No `src/` change (pytest 19). Rémy validated visually, merged to `chapter/06_RandomForest` |
| 4 | `notebook/06_RandomForest__04_estimator_and_parameters` | **done** — built (24 cells, 3 figures: OOB & test error vs `n_estimators`; single-tree vs forest test vs `max_depth`; single-tree MDI spike vs forest MDI spread), both reviewers **PASS** (no BLOCK/MAJOR). Honest parity hand-bag **0.9357** == `RF(max_features=None)` 0.9357 (accuracy match, tie-break-sensitive), `RF(sqrt)` 0.9415; `n_estimators` OOB-err 0.271→0.040 (never overfits, sklearn warns ≤10, surfaced); `max_features` OOB/CV **flat** 0.947–0.962 (forgiving, `'sqrt'` default; mechanism = NB 2's ρ); `max_depth` single-tree wobble 0.86–0.918 vs forest 0.918→0.942 plateau, run-to-run std **0.0163 vs 0.0043** (both train 1.000); MDI single-tree spike **0.740** vs forest peak **0.146** (spread; Strobl bias caveat; permutation **named** → NB 5); `GridSearchCV` `{None,'log2',1}` CV 0.957 → test 0.947 vs default 0.955/0.942 (tuning barely beats default). **`src/` add:** `viz.plot_feature_importances` + smoke test (**pytest 19→20**). Folded 3 MINOR/nit (Fig C shared-x-scale note; cell-13 train=1.000 print; "variance" not "spread" for σ²/B). No banned words, ruff/hex clean, output-free. Rémy validated visually, merged to `chapter/06_RandomForest` |
| 5 | `notebook/06_RandomForest__05_covtype_strong_baseline` | **done** — built (25 cells, 7 figures: class balance; cross-method accuracy; aggregate metrics; per-class recall; 7×7 confusion; MDI vs permutation; fit-time vs n), both reviewers **REVISE → folded** (shared MAJOR: cell-18 wrongly claimed NB 4 *measured* an MDI-vs-permutation disagreement — NB 4 only *named* permutation → reframed to NB 4's true MDI single-tree-spike→forest-spread story; MINORs: Soil = 2nd-largest signal not 3rd, "no soil col high" qualified for the perm panel, MDI/perm different scales → agree on *ranking*; added "Going further"). Anchors: covtype 30k; RF **0.844**/OOB 0.846 ≫ tree 0.770 ≫ LogReg 0.729 (the forest **wins**, reverse of breast_cancer); macro-F1 0.737 vs accuracy 0.844; Aspen recall 0.279; Elevation MDI 0.233 ≈ perm 0.270; 40 Soil one-hot combined 0.141/0.112; fit-time n^0.99 (vs SVM n^1.6). No `src/` change (`fetch_covtype` direct; pytest 20). Rémy validated visually, merged to `chapter/06_RandomForest` |
