...
| 5 | `notebook/05_SVM__05_breast_cancer_scaling_limits` | **done** — built (26 cells, 6 figures: class balance, raw-vs-std scaling bar, `C × gamma` heatmap, cross-method spine bar, confusion, fit-time-vs-`n` curve), **both reviewers PASS** (no BLOCK/MAJOR; ml-expert 3 MINOR folded — the ch-03 threshold contrast made precise (the lever reaches *borderline* misses, not confident ones, in either model), "lowering only adds positives", exponent flagged this-run; pedagogy 2 MINOR — course_map mark, Going-further optional), Rémy validated visually, merged to `chapter/05_SVM`. No `src/` change (pytest 19). |
