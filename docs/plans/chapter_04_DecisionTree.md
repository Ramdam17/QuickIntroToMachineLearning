...
| 4 | `notebook/04_DecisionTree__04_estimator_and_parameters` | **done** — built (23 cells, 3 figures: min_samples_leaf 1-vs-5, two bootstrap trees = variance headline, Gini importance bar), both reviewers **REVISE → folded** (MAJOR both: cell-13 read wrongly said the two bootstrap trees' leaf counts differ → reframed "same 17 leaves, different boundary, test 0.900 vs 0.833"; MINOR: leakage note for whole-set standardization). Rémy validated visually, merged to `chapter/04_DecisionTree` |
| 5 | `notebook/04_DecisionTree__05_breast_cancer_interpretability` | **done** — built (26 cells, 6 figures: class balance, depth-3 tree rules, cross-method bar, root-feature flips, Gini-vs-permutation, confusion), both reviewers folded (pedagogy PASS; ml-expert REVISE → 1 MAJOR fixed: cell-22 threshold conflation reframed + MINORs), Rémy validated visually, merged to `chapter/04_DecisionTree` |
